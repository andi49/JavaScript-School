...
exports.app.get('/movies', async (req, res) => {
    ...
    try {
        if (typeof req.query.id === 'string') {
            console.log('ID');
            const movie = await (0, fileoperation_1.getMovieByID)(req.query.id);
            if (movie)
                res.json(movie);
            else
        ...
        // I annat fall hämtar vi alla filmer
            ...
            const movies = await (0, fileoperation_1.getMovies)();
...
exports.app.post('/movies', async (req, res) => {
    ...
    try {
        if ((0, types_1.isNewMovie)(req.body)) {
            const newMovie = await (0, fileoperation_1.addMovie)(req.body);
...
exports.app.delete('/movies', async (req, res) => {
    try {
        if (typeof req.query.id === 'string') {
            const movie = await (0, fileoperation_1.deleteMovieByID)(req.query.id);
            if (movie)
...
exports.app.patch('/movies', async (req, res) => {
    ...
    try {
        if (typeof req.query.id === 'string' && typeof req.body.rating === 'number') {
            const movie = await (0, fileoperation_1.updateMovieRating)(req.query.id, req.body.rating);
            if (movie)
                res.json({ message: 'Movi deleted', movie });
            else
                res.status(404).json({ message: 'No Movie found with matching ID' });
